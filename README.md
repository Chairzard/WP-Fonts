...
This project started off to address a shortcoming of KOReader's engine at the time: No ability to synthesize small-caps that lacked them. As a secondary focus, I'd make improvements to the fonts I was tinkering with at the same time. As KOReader now can synthesize small-caps, the focus of the repo has flipped a bit on its head. Now, the focus is on fonts that have had technical improvements made to them (kerning fixes, enabling opentype features that weren't properly implemented, etc).
...
- The most famous free & open-source sans-serif and monospaced fonts can also be found here. However, because sans-serif fonts and monospaced fonts start to feel same-y after a while (because there's only so much a creator can do to differentiate them from the crowd), my collection there is less comprehensive.
...
*IMPORTANT NOTE: If you have an ereader with a 300+ PPI display, I **strongly** recommend turning off font hinting in KOReader for the best font rendering experience. Leaving it on can cause glyphs to distort in unusual ways! If you're on a lower PPI display, you might need the hinting, though.*

I am also keeping my vast catalog of catalog of fonts that were modified to have small-caps here rather than removing them (created in the era when KOReader didn't make its own small-caps), as their included small-caps are generally superior to KOReader's small-caps for the following reasons:

- *Personal preference for this point: The KOReader engine adds a little extra weight to its fake small-caps to make up for the weight that was lost when the glyphs were scaled down. This makes the fake small-caps blend in better with the text, but sometimes causes glyphs to dip below the baseline (which is a huge pet peeve of mine). You may well prefer this trade-off versus my own fake small-caps (which don't add weight), in which case you'd be better served by using the stock versions of those fonts.*
...
- I don't like how the engine handles scaled down small-caps for monospaced fonts, as it breaks monospacing (note: this is an issue with *all* ereading apps due to the technical limitations of faking small-caps).
- The KOReader engine scales glyphs to 75% of their height. This is a pretty decent choice, given the engine can only choose one number to scale by, but this is an imperfect solution for every font. Small-capitals should ideally be at least a font's x-height % tall, plus ideally a little taller than that (10-20% taller than the x-height is generally the ideal). If the x-height of a font is, say, 80%, the small-caps will look anemic. My fonts have had their small-caps based on the X-height (+10-20%, as needed), rather than a flat percentage, so they tend to be sized more appropriately.  

**Q: How do I install these?**  
**A:** See the user guide here: https://koreader.rocks/user_guide/#L2-fonts. In general, you can just download the latest releases and drag all the font folders into KOReader's fonts folder (tip: you can also download fonts individually by exploring the repo a bit). *Be advised that the TeX Gyre fonts (and possibly some others I've forgetten) are distributed in their original .zip files to comply with their license terms, so you'll need to extract those first before you can use them, too.*
...
|**Anaheim**|WP California|SIL Open Font License (version 1.1)|✅|❌|❌|❌||
...
|**Atkinson Hyperlegible Mono**|WP Superlegible Mono|SIL Open Font License (version 1.1)|✅|✅|✅|❌|Monospaced font.|
|**Atkinson Hyperlegible Next**|WP Superlegible|SIL Open Font License (version 1.1)|✅|✅|✅|❌||
...
|**Beholden Book**|WP Beholder Book|SIL Open Font License (version 1.1)|✅|✅|✅|✅|Fixes ligature kerning/metrics, removes some blank glyphs, and cleans up the c2sc/smcp tables.|
...
|**Literata**|WP Illiterata|SIL Open Font License (version 1.1)|✅|✅|✅|✅|Reduces the space between the `H e` kerning pair (in all styles), as well as `S k` (in the non-italic styles). Also fixes a bug with the `d .` kerning pair (in the Regular Italic file only).|
...
|**OCR-A**|WP OCR-A|SIL Open Font License (version 1.1)|❌|❌|❌|❌|Custom glyphs have also been added. Note that the character set it very limited here in terms of diacritics.|
|**OCR-B**|WP OCR-B|SIL Open Font License (version 1.1)|❌|❌|❌|❌|Custom glyphs have also been added. Note that the character set it very limited here in terms of diacritics.|
...
|**STIX Two Text**|WP Pick-up Sticks|SIL Open Font License (version 1.1)|✅|✅|✅|✅|Fixes rendering bugs with the ligatures (instead of using contextual alternates like the base font, I use its dedicated ligatures). Metrics for the ligatures were corrected, and kerning was copied over from the glyphs that make up its parts.|
...
|**Ubuntu Mono**|Ubuntu Mono derivative WP|UBUNTU FONT LICENCE|✅|✅|✅|❌|Monospaced font. The font is named differently than other fonts in this repo due to the font's license terms.|
|**Ubuntu Sans**|Ubuntu Sans derivative WP|UBUNTU FONT LICENCE|✅|✅|✅|✅|The font is named differently than other fonts in this repo due to the font's license terms. Italic/Bold Italic small caps are oblique versions of the true small caps from the regular/bold styles.|
...
**A:** Font styles will be missing when the original font lacks them (or, in very rare cases, because those style files have bad issues, such as *extremely* incomplete glyph sets or a bold weight that isn't really bold). I try to favor fonts that include all four styles, but oftentimes I liked the base font enough to include it anyway and deal with KOReader applying a fake oblique and/or bold effect when needed. Your ereader/app will probably fake these effects if they’re missing (one important exception I know of is Kindle, which won’t properly render Bold Italic if you are missing the Bold Italic File but have regular Bold and Italic; in that case, delete/don’t use the Bold file at all).
