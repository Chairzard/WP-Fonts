...
A: Most open font licenses are very nonrestrictive, with the exception of reserved font names. To legally redistribute them, I had to change the font names. If you’re so inclined, you can change the names back in FontForge; you just won’t be able to legally redistribute them, in that case.
...
If you’re looking to create your own fake small-capital glyphs, here are the steps I used in FontForge (current as of version 20251009). I strongly recommend saving frequently when doing this, as FontForge has a bad habit of crashing, especially in step 4.
    ...
    4. Select Element → Style → Add Small-Capitals and hit the OK button. This will create encoding slots for the small-capital glyphs, as well as create some that will probably look pretty bad (we’ll replace them).
        ◦ NOTE: FontForge loves to crash on this step, usually for one of two reasons:
            ▪ There’s a glyph that it doesn’t like in your glyph set. This rarely happens in Latin (you can find out which glyph is making it crash via trial and error), but it is super common in Greek with the capital letter Omega, as there are often 2 omegas in the font file (you’ll need to figure out which of the two is causing the crash via trial and error).
            ▪ Less commonly, in the “Add Small-Capitals” dialog box, an invalid number labeled “nan” may appear. If this happens, replace the number with any valid number.
    ...
    7. OPTIONAL but highly recommended if your font has kerning (most good fonts that aren’t monospaced do): Repeat steps 1-2, remove any duplicates you previously needed to remove, then use Edit → Copy → Copy Lookup Data on the selected glyphs.
        7A. Select all the small-cap glyphs and paste. Select all entries on the list that list “’kern’”. There will always be multiples of two (there’s a “second glyph” entry at the bottom of the list and a first entry somewhere above it).
    ...
    13. Go to Element → Font info → Lookups. Under GSUB, we’ll need to edit two tables so that smallcap lookups happen correctly. For both the “smcp” and “c2sc” tables, follow the following steps:
        11a. Move smcp and c2sc to the top of your list, if not there already (the order of those two doesn’t matter; those two just need to be above everything else). Hit the plus button, then click the entry that pops up, and finally click Edit Data.
        11b. IN ORDER, do the following: Remove All, populate, remove empty. You’ll know you messed this step up if you see dots about your small-capital “I”s.
OPTIONAL: If you want proper kerning for the small-capital glyphs, you’re going to have to manually add it in. Step (insert) copied over any kerning pairs your font may have had, but if your font also uses kerning tables (common, especially for newer fonts), the only way that I know of to do this is to manually add all your small-cap glyphs to the kerning classes that the glyphs they’re based off of are in (for example, you’d add “a.sc” to the Kerning classes “A” is in). I’m not going to sugarcoat it; this can be a brutal process and take way too long, so if you can live without proper kerning, skip this step. I couldn’t, so all fonts here are kerned.
    12. Go to file → Generate fonts and generate your fonts!
